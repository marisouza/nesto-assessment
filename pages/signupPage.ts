...
export class SignupPage {
    ...
    constructor(page: Page, language: 'en' | 'fr' = 'en') {
        ...
        this.consent = page.getByRole('button', { name: localeData.consentAccept, exact: true });
        this.firstNameInput = page.getByRole('textbox', { name: localeData.firstName, exact: true });
        this.lastNameInput = page.getByRole('textbox', { name: localeData.lastName, exact: true });
        this.emailInput = page.getByRole('textbox', { name: localeData.email, exact: true });
        this.phoneInput = page.getByRole('textbox', { name: localeData.phoneNumber, exact: true });
        this.passwordInput = page.getByRole('textbox', { name: localeData.password, exact: true });
        this.confirmPasswordInput = page.getByRole('textbox', { name: localeData.passwordConfirmation, exact: true });
        this.signUpButton = page.getByRole('button', { name: localeData.signUp, exact: true });
        this.loginLink = page.getByRole('link', { name: localeData.logIn, exact: true });
        this.termsOfServiceLink = page.getByText(localeData.termsOfService, { exact: true });
        this.privacyPolicyLink = page.getByText(localeData.privacyPolicy, { exact: true });
        this.errorMessages = page.locator('[data-testid*="error-message-typography"]');
        this.countrycodeSelector = page.getByRole('combobox', { name: 'phoneCountry' });
        this.regionInput = page.getByTestId('region-select');
        this.languageSelector = page.getByTestId('header-language-switch');
    }

    async getSignupUrl() {
        const signUpUrl = this.language === 'fr' ? 'https://app.qa.nesto.ca/fr/signup' : 'https://app.qa.nesto.ca/signup';
        return signUpUrl
    }
