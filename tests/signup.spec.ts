...
const runSignupTests = (lang: Language) => {
  test.describe(`Signup Page - ${lang.toUpperCase()}`, () => {
    test.describe('Form Validation', () => {
      test('should not submit form and show inline errors when mandatory fields are empty', async ({ signupPage }) => {
      ...
      test.skip('should show line error when phone number and country selection are different', async ({ signupPage }) => {
        ...
        const locator = await signupPage.page.getByText(await signupPage.getLocaleText("invalidPhoneNumber"));
        await expect(locator).toBeVisible();
        await expect(locator).toHaveText(await signupPage.getLocaleText("invalidPhoneNumber"));
      });

      test('should show invalid phone number inline error ', async ({ signupPage }) => {
        const invalidPhone = faker.phone.number({ style: 'national' }).slice(0,7);
        await signupPage.fillInputField(signupPage.phoneInput, invalidPhone);
        await signupPage.submitSignupForm();

        expect(signupPage.phoneInput).toHaveValue(invalidPhone);
        const locator = signupPage.page.getByText(await signupPage.getLocaleText("invalidPhoneNumber"));
        await expect(locator).toBeVisible();
        await expect(locator).toHaveText(await signupPage.getLocaleText("invalidPhoneNumber"));
      });

      test('should show invalid email address inline error', async ({ signupPage }) => {
        ...
        const error = await signupPage.getErrorMessageByTestId('email-error-message-typography');
        const expectedText = await signupPage.getLocaleText("invalidEmail");
        expect(error).toContain(expectedText);
      });
      
      test('should show password mismatch inline error', async ({ signupPage }) => {
        await signupPage.fillInputField(signupPage.passwordInput, 'Password1234');
        await signupPage.fillInputField(signupPage.confirmPasswordInput, 'Different123');
        await signupPage.submitSignupForm();

        const error = await signupPage.getErrorMessageByTestId('passwordConfirmation-error-message-typography');
        const expectedText = await signupPage.getLocaleText("passwordsMismatch");
        expect(error).toEqual(expectedText);
      });

      test('should show password below minimum length inline error', async ({ signupPage }) => {
        await signupPage.fillInputField(signupPage.passwordInput, 'Pass1');
        await signupPage.submitSignupForm();

        const error = await signupPage.getErrorMessageByTestId('password-error-message-typography');
        const expectedText = await signupPage.getLocaleText("passwordsMinimumRequirement");
        expect(error).toEqual(expectedText);
      });

      test('should show password above maximum length inline error', async ({ signupPage }) => {
        const tooLongPassword = 'A'.repeat(31) +'1a';
        await signupPage.fillInputField(signupPage.passwordInput, tooLongPassword);
        await signupPage.submitSignupForm();

        const expectedText = await signupPage.getLocaleText("passwordMaximumRequirement");
        expect(await signupPage.getErrorMessageByTestId('password-error-message-typography')).toEqual(expectedText);
      });

      test('should not show password minimum chars length inline error', async ({ signupPage }) => {
        await signupPage.fillInputField(signupPage.passwordInput, 'Password1234');
        await signupPage.submitSignupForm()
        await expect(await signupPage.getLocatorByTestId('password-error-message-typography')).toBeHidden();
      });

      test('should not show password maximum chars length inline error', async ({ signupPage }) => {
      ...
      test('should show password does not match requirements inline error', async ({ signupPage }) => {
        await signupPage.fillInputField(signupPage.passwordInput, 'password1234');
        await signupPage.submitSignupForm();
        
        const expectedText = await signupPage.getLocaleText("passwordRequirements");
        const error = await signupPage.getErrorMessageByText(expectedText);
        expect(error).toEqual(expectedText);
      });

      test('should not show error when password contains special chars(?!#_- ) but meets all requirements', async ({ signupPage }) => {
        await signupPage.fillInputField(signupPage.passwordInput, 'Password123?!#_');
        await signupPage.submitSignupForm();
        await expect(await signupPage.getLocatorByTestId('password-error-message-typography')).toBeHidden();
      });
    });
    ...
    test.describe('Links and Navigation', () => {
      test('should switch language when clicking the language toggle', async ({ signupPage }) => {
        const targetLanguage = lang === 'en' ? 'fr' : 'en';
        const expectedUrlPart = targetLanguage === 'fr' ? '/fr/signup' : '/signup';
        const headerText = targetLanguage === 'fr' ? 'Créez un compte nesto' : 'Create a nesto account';

        await signupPage.languageSelector.click();
        await signupPage.page.waitForResponse(resp => 
                resp.url().includes(`/${targetLanguage}/signup.json`) && resp.request().method() === 'GET' && resp.status() === 200);
        await expect(signupPage.page).toHaveURL(new RegExp(expectedUrlPart));
        expect(signupPage.page.getByText(headerText, { exact: true })).toBeTruthy();
      });

      test('should redirect to login page when clicking the login link', async ({ signupPage }) => {
        ...
        await expect(signupPage.page.getByRole('button', { name: expectedText })).toBeVisible();
      });

      test('should have correct link for terms of service', async ({ signupPage }) => {
        const href = await signupPage.getTermsOfServiceHref();
        const expectedText = await signupPage.getLocaleText("termsOfServiceLink");
        expect(href).toContain(expectedText);
      });

      //BUG: Test is skipped until the privacy policy link is for EN users is fixed
      test.skip('should have correct link for privacy policy', async ({ signupPage }) => {
        const href = await signupPage.getPrivacyPolicyHref();
        const expectedText = await signupPage.getLocaleText("privacyPolicyLink");
        expect(href).toContain(expectedText);
      });
      ...
      test('should display toast error when signup using email from existing user', async ({ signupPage }) => {
        ...
        const locator = signupPage.page.getByText(expectedText, { exact: true });

        await signupPage.page.waitForSelector('.Toastify', { state: 'attached' });
        await expect(locator).toHaveText(expectedText);
      });

      test('should not allow signup with malicious strings in email/password', async ({ signupPage }) => {
        ...
        await signupPage.page.getByRole('checkbox', { name: await signupPage.getLocaleText("checkBoxTerms") }).check();
    ...
    test('should successfully submit form', async ({ signupPage }) => {
      const randomUserFirstName = faker.person.firstName();
      const randomUserLastName = faker.person.lastName();
      const randomEmail = faker.internet.email({firstName: randomUserFirstName, lastName: `${randomUserLastName}${Date.now().toFixed()}`});
      ...
      await signupPage.submitSignupForm();
      const accountResponse = await signupPage.page.waitForResponse(resp =>
        resp.url().includes('/api/accounts') && resp.request().method() === 'POST' && resp.status() === 201
      );

      const accountResponseBody = await accountResponse.json();
      expect(accountResponseBody.account.firstName).toEqual(randomUserFirstName);
      ...
      const url = selectedLanguage === 'fr' ? 'https://app.qa.nesto.ca/getaquote/fr' : 'https://app.qa.nesto.ca/getaquote';
