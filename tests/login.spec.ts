...
const runSignupTests = (lang: Language) => {
  test.describe(`Login Page - ${lang.toUpperCase()}`, () => {
    ...
    test.skip('should not login when username provided is incorrect for an existing user', async () => {
      ...
      await expect(loginPage.page.getByText('We\'re sorry, something went')).toBeAttached();
    ...
    test('should show inline errors when empty fields are submitted', async () => {
      ...
      const emailExpectedError = await loginPage.getLocaleText("loginBlankEmailError");
      const passwordExpectedError = await loginPage.getLocaleText("loginBlankPasswordError");
