...
export async function applyConsent2(page: Page) {
        ...
        const didomi = process.env.DIDOMI? process.env.DIDOMI : JSON.parse(data).didomi_token;
        const eu = process.env.EUCONSENT? process.env.EUCONSENT : JSON.parse(data)['euconsent-v2'];
        ...
        await page.addInitScript(
            ([didomi, eu]) => {
                try {
                    ...
                    console.error('Could not set localStorage didomi and euconsent-v2 items for consent:', e);
                }
            },
            [didomi, eu]
        );
